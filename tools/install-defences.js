...
//   - Ensures husky is listed in devDependencies.
...
const SCRIPTS_TO_ADD = {
  ...
  'defence:reinstall': 'node --version && npm --version && npm run defence:pkg-age-check && rm -rf node_modules && npm cache clean --force && npm ci && npm audit signatures && npm run prepare && node ./tools/check-package-age.js --transitive && npm audit fix && node ./tools/check-package-age.js --transitive && npm outdated',
  ...
  test: 'node --test tools/check-package-age.test.js',
  prepare: 'husky',
}
...
function showUsage() {
  console.log('Usage:')
  console.log('  node ./tools/install-defences.js <path-to-target-project> [--dry-run] [--force]')
  console.log()
  console.log('Options:')
  console.log('  --dry-run  Show what would be copied without changing any file.')
  console.log('  --force    Overwrite existing files (a backup is created first).')
}

function readTargetPackageJson(targetDir) {
  const pkgPath = path.join(targetDir, 'package.json')
  if (!fs.existsSync(pkgPath)) {
    throw new Error(`Target directory does not contain a package.json file: ${targetDir}`)
  }
...
function copyFile(src, dest, force, dryRun) {
  if (fs.existsSync(dest)) {
    if (!force) {
      throw new Error(`File already exists: ${dest}. Use --force to overwrite (backup will be created).`)
    }
    if (!dryRun) {
...
function updatePackageJson(pkgPath, content, dryRun) {
  ...
  const conflicts = Object.keys(SCRIPTS_TO_ADD).filter((key) => originalScripts[key] && originalScripts[key] !== SCRIPTS_TO_ADD[key])
  ...
  const newDevDeps = { ...(content.devDependencies || {}), husky: '9.1.7' }
  ...
  if (dryRun) {
    console.log('  [dry-run] Would update scripts and devDependencies in package.json')
    return
  }

  fs.writeFileSync(pkgPath, `${JSON.stringify(updated, null, 2)}\n`)
  console.log('  Updated package.json with defence scripts and husky devDependency.')
}
...
function main(argv) {
  ...
  console.log(`${dryRun ? '[dry-run] ' : ''}Installing supply-chain defences into ${targetDir}`)
  ...
  if (dryRun) {
    ...
    console.log('  1. Run npm install to install husky and generate the lock file.')
    console.log('  2. Run bash .husky/pre-commit to verify the hook.')
    console.log('  3. Commit .npmrc, .husky/, tools/, and package.json changes.')
  }
  return 0
}
